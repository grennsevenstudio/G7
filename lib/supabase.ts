...
export const authenticateUser = async (email: string, password?: string): Promise<{ user: User | null, error: any }> => {
    try {
        ...
        if (error) {
            ...
            if (isNetworkError(error)) return { user: null, error: { message: 'Network Error', isNetwork: true } };
        ...
        const mappedUser: User = {
            ...
            plan: data.plan || 'Conservador',
            lastPlanChangeDate: data.last_plan_change_date,
            balanceUSD: Number(data.balance_usd || 0),
            capitalInvestedUSD: Number(data.capital_invested_usd || 0),
            monthlyProfitUSD: Number(data.monthly_profit_usd || 0),
            dailyWithdrawableUSD: Number(data.daily_withdrawable_usd || 0),
            bonusBalanceUSD: Number(data.bonus_balance_usd || 0),
            lastProfitUpdate: extra.lastProfitUpdate || data.created_at || new Date().toISOString(),
            isAdmin: data.is_admin === true,
            joinedDate: data.created_at ? new Date(data.created_at).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
            referralCode: data.referral_code || extra.referralCode || '',
            referredById: data.referred_by_id || extra.referredById,
            transactionPin: data.transaction_pin || extra.transactionPin,
            supportStatus: data.support_status || 'open'
        };

        return { user: mappedUser, error: null };
    } catch (e) {
        const { error } = handleSupabaseError(e, 'authenticateUser');
        return { user: null, error };
    }
};
